...
public class TcpServer extends AbstractLifecycle implements NetworkService {
    ...
    public TcpServer(TcpProperties tcpProperties, TcpServerHandler channelHandler, MessageDispatcher dispatcher) {
    ...
    public void start() {
        ...
        // 检查协议格式是否设置正确
        if (Arrays.stream(ProtoEnum.values()).noneMatch(p -> p.name().equalsIgnoreCase(protocol))) {
        ...
        bootstrap.group(bossGroup, workerGroup)
                ...
                .childHandler(new ChannelInitializer<Channel>() {
                    ...
                    protected void initChannel(Channel ch) {
                        ...
                        if (ProtoEnum.JSON.name().equalsIgnoreCase(protocol)) {
                            p.addLast(new JsonObjectDecoder());
                            p.addLast(new JsonDecoder());
                            p.addLast(new JsonEncoder());
                        } else if (ProtoEnum.PROTOBUF.name().equalsIgnoreCase(protocol)) {
                            // TODO 放开它们
